use super::{
    list_remote_sdk_cache::RemoteSdkListCache,
...
struct SdkServiceInner {
    remote_sdk_list_cache: RemoteSdkListCache,
    local_repository: LocalSdkRepository,
    remote_repository: RemoteSdkRepository,
}

impl SdkServiceInner {
    fn new() -> Self {
        Self {
            remote_sdk_list_cache: RemoteSdkListCache::new(),
    ...
    fn get_available_remote_sdk_list(
        ...
        if let Some(sdks) = self.remote_sdk_list_cache.load_list(context, clock) {
            debug!("sdk list from cache");
            return anyhow::Ok(sdks);
        }

        let result = self.remote_repository.fetch_available_sdk_list(git_command);
        if let Ok(sdks) = &result {
            debug!("sdk list from remote");
            if let Err(e) = self.remote_sdk_list_cache.store_list(context, clock, sdks) {
                warn!("{e}");
            }
        }
        result
...
pub struct RealSdkService {
    dependencies: SdkServiceDependencies<GitCommandImpl, SystemClock>,
