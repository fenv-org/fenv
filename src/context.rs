...
pub trait FenvContext<'a> {
    ...
    /// otherwise, `$HOME/.fenv`.
    fn fenv_root(&'a self) -> &'a PathLike;
    ...
    ///
    /// `{fenv_root}/versions`.
    fn fenv_versions(&self) -> &'a PathLike {
    ...
    ///
    /// `{fenv_root}/version`.
    fn fenv_global_version_file(&self) -> &'a PathLike {
        self.fenv_root.join("version")
    }
}

pub struct FenvContextImpl<'a> {}

impl<'a> FenvContext<'a> for FenvContextImpl<'a> {}
